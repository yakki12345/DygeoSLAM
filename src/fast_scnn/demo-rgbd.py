...
class Mask:
    ...
    def GetDynSeg(image):
        ...
        mask=np.array(mask)
        #cv2.imwrite('/home/yakki/DynaSLAM_LK/data/test/test.png',mask)
        ...
        for i in range(mask.shape[0]):
            for j in range(mask.shape[1]):
                if((mask[i][j][0]==220)and(mask[i][j][1]==20)and(mask[i][j][2]==60)):
    	            ...
    	            # re[i][j]=1
        # re2=np.zeros((mask.shape[0],mask.shape[1]))
        # r_img = mask[:,:,0].copy()
        # g_img = mask[:,:,1].copy()
        # b_img = mask[:,:,2].copy()
        # img = r_img * 256 * 256 + g_img * 256 + b_img
        ...
        # re[img==src_c]=1
        # re[img==src_b]=1
        cv2.imwrite('/home/yakki/DynaSLAM_LK/data/test/test.png',re)
        return re
    def say():
        print("hello")
        print("why i cannot")

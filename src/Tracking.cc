...
{   cout<<"this is stereo"<<endl;
    ...
    // mCurrentFrame.CalculEverything(mImGray,mImDepth);
    Track();
    cout<<"track end"<<endl;
...
{   cout<<"this is rgbd"<<endl;
    ...
    if(!needNewKF){//如果当前帧不是关键帧，那么就不需要用maks来判断哪些特征点不能要，直接用前一个关键帧的特征点进行光流跟踪
        clock_t a = clock();
...
//        LKimg = flow(mpLastKeyFrame, imRGB,  mCurrentFrame.mnId - mpLastKeyFrame->mnFrameId ==1, mK, mDistCoef, mTcw);
        ...
        mCurrentFrame.mTcw = mTcw;//直接用PnpRANSAC计算位姿，等于不用lighttrack()和几何法判断动态点了。
        //light_track()和几何法判断动态点的方法只在于关键帧的动态特征点提前。
        clock_t b = clock();

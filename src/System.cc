        ...
        mbDeactivateLocalizationMode(false)
{
    ...
    // 2024.1.3write for point cloud resolution
...
*/
    ...
    cout<<"get out of system_ini"<<endl;
    cout<<"now in stereo2"<<endl;
    cout << "Loading Fast-scnn. This could take a while..." << endl;
...
cv::Mat System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const cv::Mat &maskLeft, const cv::Mat &maskRight,const double &timestamp)
{
    ...
    cout<<"trackstereo end"<<endl;
    return Tcw;
}
cv::Mat System::TrackStereo2(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp)//针对ROS2接口的函数，接受左右两张图片，调用网络进行分割得到mask,再调用TrackStereo
{// Initialize Mask R-CNN
 ...
 // Segment out the images
    ...
    // cout << "now we start to seg" << endl;
    // setenv("PYTHONPATH", "/home/yakki/DynaSLAM_LK/src/fast_scnn/", 1);//添加python的环境变量，使得系统可以找到python文件
    // std::string x = getenv("PYTHONPATH");
    // std::cout<<x<<std::endl;
    // Py_Initialize();
    // if(!Py_IsInitialized())
    //     std::cout << "[Error] Init error" << std::endl;
    // std::cout << "py_initialized" << std::endl;
    // DynaSLAM::NDArrayConverter *cvt=new DynaSLAM::NDArrayConverter();
    // std::cout << "cvt_created" << std::endl;
    // PyObject* py_image = cvt->toNDArray(imLeft.clone());
    
    // if(py_image != NULL)
    //     std::cout << "py_image" << std::endl;
    // //PyRun_SimpleString("import sys");
    // //PyRun_SimpleString("sys.path.append('/home/yakki/DynaSLAM_LK/src/fast_scnn/')");
    // PyRun_SimpleString("import sys");
    // PyRun_SimpleString("print(sys.executable)");
    // PyObject* pMo = PyImport_ImportModule("ttt");
    // std::cout << "this is nnnn" << std::endl;
    // if(pMo==NULL)
    ...
    // }
    // PyObject* pmm = PyObject_GetAttrString(pMo, "GetDynSeg");
    // if(pmm== NULL)
    //     std::cout << "pmm is null" << std::endl;
    // else
    //     std::cout << "pmm is not null" << std::endl;
    // std::cout << "now we call method" << std::endl;
    // PyObject* py_mask_imagel = PyObject_CallObject(pmm,py_image);
    // cv::Mat seg =cvt->toMat(py_mask_imagel).clone();
    // assert(seg != NULL);
    // seg.cv::Mat::convertTo(seg,CV_8U);
    // maskLeftRCNN=seg;
    cout<<"now begin to segment"<<endl;
    maskLeftRCNN = MaskNet->GetSegmentation(iml, "/home/yakki/DynaSLAM_LK/imLeft",
            to_string(timestamp)+".png");
    maskRightRCNN = MaskNet->GetSegmentation(imr, "/home/yakki/DynaSLAM_LK/imRight",
            to_string(timestamp)+".png");
    cout << "now we end seg" << endl;
    ...
    return TrackStereo(imLeft, imRight, maskLeft, maskRight, timestamp);
